...
export default function ManagePage() {
  ...
  const intervalLabel = member?.billing_interval === 'yearly' ? 'year' : 'month';
        ...
        <div className="bg-white rounded-2xl border border-warm-200 p-8">
          {loading ? (
            <div className="text-center">
              ...
              <p className="text-forest-800 font-medium">Loading your membership…</p>
            </div>
          ) : error ? (
            <div className="text-center">
              ...
              <h1 className="font-display font-bold text-2xl text-forest-900 mb-2">Invalid link</h1>
              <p className="text-gray-500 text-sm">This manage link is invalid or has expired. Please contact your gym administrator.</p>
            </div>
          ) : member ? (
            <>
              <div className="mb-6">
                <p className="text-sm text-gray-500 mb-1">Membership at</p>
              ...
              {member.status === 'cancelled' && (
                <div className="bg-warm-50 border border-warm-200 rounded-xl p-4 text-sm text-gray-600 text-center">
                  Your membership has been cancelled. Contact your gym to rejoin.
                </div>
              )}

              {member.status === 'pending' && (
                <div className="bg-warm-50 border border-warm-200 rounded-xl p-4 text-sm text-gray-600 text-center">
                  Your membership is pending payment. Check your email for the payment link.
                </div>
              )}

              {member.status === 'suspended' && (
                <div className="bg-warm-50 border border-warm-200 rounded-xl p-4 text-sm text-gray-600 text-center">
                  Your membership has been suspended. Please contact your gym administrator.
                </div>
              )}
